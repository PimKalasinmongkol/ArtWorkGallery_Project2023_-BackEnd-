...
router.get('/search_gallery/:search_text', async(request, response) => {
    let search_text = request.params.search_text

    try {
        const client = await pool.connect()
        const result = await client.query("SELECT * FROM public.gallery WHERE gallery_title LIKE $1 ORDER BY gallery_id DESC", [`%${search_text}%`])
        const data = await result.rows
        client.release()
        response.status(200).json(data)
    } catch (error) {
